...
public sealed class Linq2DbPlayerRepository : IPlayerRepository
{
    ...
    public async Task<IReadOnlyList<Player>> ListByAccountAsync(Guid accountId, CancellationToken cancellationToken)
    {
        List<PlayerEntity> rows = await _db.Players
    ...
    public async Task<Player?> GetByRoomAndAccountAsync(Guid roomId, Guid accountId, CancellationToken cancellationToken)
    {
        PlayerEntity? playerRow = await _db.Players.FirstOrDefaultAsync(
            p => p.RoomId == roomId && p.AccountId == accountId,
            cancellationToken);
    ...
    public async Task<Player?> GetByRoomAndPlayerCodeHashAsync(Guid roomId, string playerCodeHash, CancellationToken cancellationToken)
    {
        PlayerEntity? playerRow = await _db.Players.FirstOrDefaultAsync(
            p => p.RoomId == roomId && p.PlayerCodeHash == playerCodeHash,
            cancellationToken);
    ...
    public async Task SaveAsync(Player player, CancellationToken cancellationToken)
    {
        ...
        await _db.Players
            ...
            .Set(p => p.AccountId, player.AccountId)
    ...
    private async Task InsertPlayerGraphAsync(Player player, CancellationToken cancellationToken)
    {
        await _db.InsertAsync(new PlayerEntity
        {
            ...
            PlayerCodeHash = player.PlayerCodeHash,
    ...
    private async Task<Player> MapPlayerAsync(PlayerEntity playerRow, CancellationToken cancellationToken)
    {
        ...
        return new Player
        {
            ...
            PlayerCodeHash = playerRow.PlayerCodeHash,
